...
export default function EditPersonaPage() {
    ...
    const fetchPersona = async () => {
        try {
            ...
            setOccupation(persona.occupation || '');
            setAge(persona.age ? persona.age.toString() : '');
            setDescription(persona.description || '');
            setBackground(persona.background || '');
            setCategory(persona.category || 'custom');
            setTags(persona.tags || '');

            // Parse and set voice profile
            if (persona.voiceProfile) {
                const voice = JSON.parse(persona.voiceProfile);
                setSpeakingStyle(voice.speakingStyle || '');
                setVocabularyLevel(voice.vocabularyLevel || '');
                setSentenceStructure(voice.sentenceStructure || '');
                setCatchphrases(voice.catchphrases ? voice.catchphrases.join(', ') : '');
                setSpeechRhythm(voice.speechRhythm || '');
            }

            // Parse and set beliefs
            if (persona.beliefs) {
                const beliefs = JSON.parse(persona.beliefs);
                setPolitical(beliefs.political || '');
                setMoral(beliefs.moral || '');
                setPhilosophy(beliefs.philosophy || '');
                setPrinciples(beliefs.principles ? beliefs.principles.join(', ') : '');
            }

            // Parse and set tone profile
            if (persona.toneProfile) {
                const tone = JSON.parse(persona.toneProfile);
                setDefaultMood(tone.defaultMood || '');
                setEmotionalRange(tone.emotionalRange || '');
                setHumorStyle(tone.humorStyle || '');
                setFormalityLevel(tone.formalityLevel || '');
            }

            // Parse and set behaviors
            if (persona.behaviors) {
                const behaviors = JSON.parse(persona.behaviors);
                setDecisionMaking(behaviors.decisionMaking || '');
                setConflictResponse(behaviors.conflictResponse || '');
                setSocialPreferences(behaviors.socialPreferences || '');
                setWorkEthic(behaviors.workEthic || '');
                setCopingMechanisms(behaviors.copingMechanisms || '');
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            router.push(`/persona-gpt/${params.id}`);
        } catch (err) {
            ...
            <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-10">
                <div className="mb-8">
                    ...
                    <p className="mt-2 text-gray-600">
                        Update the voice, beliefs, and behaviors of this persona
                    </p>
                </div>
